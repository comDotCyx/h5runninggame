define(["lib/zepto_amd.min", "app/gameConfig", "app/movers", "app/player", "app/collision", "app/touch", "lib/gameFunctions", "lib/localStorage", "lib/zepto.imgpreload"], function($, getGameConfig, Movers, Player, obstaclesCollision, touchEventsListeners, gamefuncs, myLocalStorage, imgPreload){
	...
	var audioSrc = ["../media/bgm1.mp3","../media/bgm1.ogg"];
	var audioTimer = null;
	var bgmBtn = $("#bgmBtn");
	var canPlayType;
	...
	function gameLoop()
	{
		...
		for(var i=0;i<moversNum-1;i++)
		{
			moveObjs[i].Monitor(deltaTime);
			moveObjs[i].draw(deltaTime);
		}
		...
		//绘制人物
		player.draw(ctx2,40,deltaTime);

		// 把障碍物放到人物之后画		
		if(gogogo){
			obstacles.draw(deltaTime);
		}		
